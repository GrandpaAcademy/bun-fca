...
// Custom CookieJar shim to match legacy request.jar sync API
class BunCookieJar {
    constructor() {
    ...
    setCookie(cookie, url, options, cb) {
        ...
        try {
            const result = this._jar.setCookieSync(cookie, url, options || {});
    ...
    getCookieString(url, options, cb) {
        ...
        try {
            const str = this._jar.getCookieStringSync(url, options || {});
            if (cb) cb(null, str);
            return str;
        } catch (e) {
            if (cb) cb(e);
...
// Bun-native fetch-based request shim
var request = async function(options) {
    ...
    // Handle Cookies from Jar
    if (jar && typeof jar.getCookieString === 'function') {
        const cookieString = await bluebird.fromCallback(cb => jar.getCookieString(url, cb));
        if (cookieString) {
    ...
    if (options.form) {
        ...
        // Multipart handled by FormData
        const form = new FormData();
        for (const key in options.formData) {
            const val = options.formData[key];
            if (val && typeof val.append === 'function') {
                // If it's already a stream/blob-like
                form.append(key, val);
            } else if (typeof val === 'object' && val.value && val.options) {
                // request-style formData object
        ...
        body = form;
        // Don't set Content-Type, let fetch handle the boundary
    ...
    const res = await fetch(url, {
        ...
        headers,
        body,
        redirect: 'manual', // Original code handles redirects manually
    ...
    if (jar && typeof jar.setCookie === 'function') {
        ...
        if (setCookie) {
            ...
            for (const cookie of cookies) {
                try {
                    await bluebird.fromCallback(cb => jar.setCookie(cookie, url, cb));
                } catch (e) {
...
request.defaults = function() {
    return request; // Basic implementation of defaults
};

// Promisify for compatibility if needed (redundant since we used async but the code expects .then)
request = bluebird.method(request);
...
function setProxy(url) {
    if (typeof url == "undefined") return request = bluebird.promisify(require("request").defaults({ jar: true, family: 4 }));
    return request = bluebird.promisify(require("request").defaults({ jar: true, proxy: url, family: 4 }));
}
