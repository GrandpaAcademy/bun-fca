...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
    ...
    if (appState) {
        ...
        try {
            appState.forEach(c => {
                const expires = c.expires ? `; expires=${c.expires}` : "";
                const domain = c.domain ? `; domain=${c.domain}` : "";
                const path = c.path ? `; path=${c.path}` : "";
                const str = `${c.key}=${c.value}${expires}${domain}${path};`;
                jar.setCookie(str, "https://" + (c.domain || "facebook.com"));
            });
    ...
    mainPromise = mainPromise
        ...
        .then(res => {
            ...
            if (!Obj) return; // buildAPI already logged the error
            ctx = Obj.ctx;
            api = Obj.api;
            return res;
        });
